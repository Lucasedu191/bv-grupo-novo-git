...
<<<<<<< HEAD
    <p><?php echo esc_html($result['total']); ?> registro(s).</p><table class="widefat striped"><thead><tr><th>Data</th><th>Grupo</th><th>1 diária</th><th>3 dias</th><th>7 dias</th><th>15 dias</th><th>Proteção Básica</th><th>Proteção Premium</th><th>Taxa de limpeza</th></tr></thead><tbody><?php if(!$result['rows']): ?><tr><td colspan="9">Nenhum histórico encontrado para os filtros informados.</td></tr><?php endif; foreach($result['rows'] as $row): ?><tr><td><?php echo esc_html(DateTimeImmutable::createFromFormat('!Y-m-d',$row['data_referencia'])->format('d/m/Y')); ?></td><td><?php echo esc_html(self::label($row['grupo_id'])); ?></td><?php foreach(['valor_diaria','valor_3_dias','valor_7_dias','valor_15_dias','protecao_basica','protecao_premium','taxa_lavagem'] as $field): ?><td><?php echo esc_html($row[$field]===null?'Indisponível':'R$ '.number_format((float)$row[$field],2,',','.')); ?></td><?php endforeach; ?></tr><?php endforeach; ?></tbody></table>
=======
